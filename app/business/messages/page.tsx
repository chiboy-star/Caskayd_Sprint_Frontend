...
// --- TYPES ---
// Updated to match your console logs more closely
interface User {
    id: string;
    username?: string;
    email?: string;
...
export default function BusinessMessagesPage() {
  ...
  useEffect(() => {
    const fetchConversations = async () => {
        try {
            ...
            console.log("🐛 [DEBUG] Fetching Business Conversations...");
  ...
  useEffect(() => {
    ...
    fetchMessages(false);
    const poller = setInterval(() => fetchMessages(true), 3000); // Silent refresh
  ...
  const handleSendMessage = async () => {
      ...
      try {
          ...
          if (res.ok) {
              ...
              console.log("✅ [DEBUG] Message Sent:", savedMessage);
  ...
  // --- LOGIC: AM I THE BUSINESS? ---
  const isMe = (msg: Message) => {
      if (!activeConversation) return false;
      
      // Based on logs: The top level business object has the ID
      const myId = activeConversation.business?.id;
  ...
  // Helper to get Creator Name (Fallback logic)
  const getCreatorName = (conv?: Conversation) => {
      if (!conv) return "Creator";
      return conv.creator?.user?.username || 
             conv.creator?.user?.email || 
             "Creator";
  };
                            ...
                            return (
                                ...
                                >
                                    <div className="w-12 h-12 rounded-full flex-shrink-0 relative overflow-hidden border border-gray-200 md:border-2 md:border-white bg-white flex items-center justify-center text-gray-500 font-bold text-lg">
                                        {chat.creator?.profileImageUrl ? (
                                            <Image src={chat.creator.profileImageUrl} alt="C" fill className="object-cover" />
                                        ) : (
                                            name[0]?.toUpperCase()
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-baseline mb-1">
                                            <h4 className="font-bold text-gray-900 text-sm md:text-base truncate">{name}</h4>
                    ...
                    <>
                        ...
                        <div className="px-4 md:px-6 py-3 md:py-4 border-b border-gray-100 flex justify-between items-center bg-white z-20 relative shadow-sm md:shadow-none">
                            <div className="flex items-center gap-3">
                                ...
                                {/* Creator Avatar */}
                                <div className="w-10 h-10 bg-gray-200 rounded-full overflow-hidden border border-gray-100 flex items-center justify-center text-gray-600 font-bold relative">
                                     {activeConversation.creator?.profileImageUrl ? (
                                        <Image src={activeConversation.creator.profileImageUrl} alt="C" fill className="object-cover" />
                                     ) : (
                                        getCreatorName(activeConversation)[0]?.toUpperCase()
                                     )}
                                </div>
                                <div>
                                    <h3 className="font-bold text-gray-900 text-sm md:text-base">
                                        {getCreatorName(activeConversation)}
                                    </h3>
                                ...
                                messages.map((msg) => {
                                    ...
                                    return (
                                        <div key={msg.id} className={`flex w-full ${sentByMe ? "justify-end" : "justify-start"}`}>
                                            ...
                                            {/* Creator Avatar (Left Side for their messages) */}
                                            {!sentByMe && (
                                            ...
                                            <div className="flex flex-col max-w-[80%] md:max-w-[65%]">
                                                <div className={`px-4 py-3 text-sm leading-relaxed shadow-sm break-words ${
                                                    sentByMe 
                                                    ? "bg-[#5B4DFF] text-white rounded-2xl rounded-br-none" // Me (Business): Purple
                                                    : "bg-white text-gray-800 rounded-2xl rounded-bl-none border border-gray-100" // Them (Creator): White
                                                }`}>
                                                    {msg.content}
                                                </div>
